...
The complete final SQL source is preserved here:

[final-source-code/Final Source Code.sql](<final-source-code/Final Source Code.sql>)

The same work is also split into numbered files inside [sql/](sql/) so it is easier to review and run in SSMS. The question numbers are continuous from `Q1` to `Q61` across the final source, split SQL files, question list, and GUI.
...
| Path | Purpose |
| --- | --- |
| [final-source-code/](final-source-code/) | Complete final SQL source file. |
...
The root [index.html](index.html) redirects to [gui/index.html](gui/index.html). The GUI can be opened directly in a browser without installing anything.

Webpage: [https://haris617.github.io/esports-tournament-dbms/](https://haris617.github.io/esports-tournament-dbms/)
...
- Player membership in teams through `tbl_TeamPlayers`.
...
| Update | `UPDATE` queries for player, team, tournament, match, and prize-related records. |
| Delete | `DELETE`, soft delete/status update, `TRUNCATE`, and `DROP` practice operations. |
...
- Match/prize cross-check query using `EXISTS` and `NOT EXISTS`
...
- Procedure with `IF ELSE` to check whether a team has won any prize
...
2. Replace the project root path at the top of `run_all.sql`:
...
- `questions.md` and the GUI query explorer match the same `Q1` to `Q61` sequence.
